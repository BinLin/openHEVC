...
#ifdef TEST_MV_POC
        if (a_pf == PF_BI) {
            return MATCH(poc[0]) && MATCH(mv[0].x) && MATCH(mv[0].y) &&
                   MATCH(poc[1]) && MATCH(mv[1].x) && MATCH(mv[1].y);
        } else if (a_pf == PF_L0) {
            return MATCH(poc[0]) && MATCH(mv[0].x) && MATCH(mv[0].y);
        } else if (a_pf == PF_L1) {
            return MATCH(poc[1]) && MATCH(mv[1].x) && MATCH(mv[1].y);
        }
#else
        if (a_pf == PF_BI) {
...
#ifdef TEST_MV_POC
        TAB_MVF(x, y).poc[pred_flag_index] == refPicList[ref_idx_curr].list[ref_idx]
#else
